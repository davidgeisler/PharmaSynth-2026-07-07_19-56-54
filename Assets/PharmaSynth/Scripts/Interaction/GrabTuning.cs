...
public static class GrabTuning
{
    ...
    /// True when the grab already has the collision-respecting configuration.
    public static bool IsTuned(XRGrab grab)
        => grab != null && grab.movementType == XRBase.MovementType.VelocityTracking;

    /// Apply the velocity-tracked profile. Returns true when a change was made
    /// (false = null or already tuned), so callers/menus can count real work.
