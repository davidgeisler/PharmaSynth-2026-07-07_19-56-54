...
- [ ] **Dr. Jimenez**: hold LIFTED 2026-07-10 (user: generate him with AI credits; client model can still replace later). Pipeline: reference image (Claude) -> Tripo P1 image->3D (user clicks — MCP instance-ID bug) -> Tripo Rigging v1 (biped) -> idle/walk/talk animations via text-to-motion. Then `ExaminerNPC` behaviour.
...
- [ ] PPE completion: gloves + goggles as clickable items like the coat (**AI-generate the goggle + glove models — approved 2026-07-10**); coat visibly ON the player (worn visual / mirror reflection); glove material swap on hands. Watch 3D model also approved for generation.
